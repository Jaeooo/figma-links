...
Serves **`.cursor/tools/figma-explorer.html`** via **`.cursor/tools/serve_figma_explorer.py`**, opens it in the browser, lets you pick top-level frames, and saves a Markdown file to **`figma_exports/`** in the project root.
...
- Files present: **`.cursor/tools/figma-explorer.html`**, **`.cursor/tools/serve_figma_explorer.py`**
- `FIGMA_TOKEN` env var must be set (via `.env` file or Cursor environment settings).
...
- **Port conflict**: Run `python3 .cursor/tools/serve_figma_explorer.py 9000` and update the URL accordingly.
